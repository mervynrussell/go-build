...
export default class GoBuildView extends EtchComponent {
  ...
  render() {
    ...
    return <div className='go-build-panel' >
      <div id='go-build-panel-button-bar' className='go-build-panel-button-bar'>
        ...
        <div className='go-build-block'>
          <button className='btn btn-primary icon icon-home' onclick={this.handleDefaultCick} title='Revert to default settings' >Defaults</button>
        </div>
      </div>
  ...
  handleFlagChange(e) {
    ...
    if (e.target.checked) {
      this.props.store.buildArgs[e.target.id] = e.target['data-flag']
    } else {
  ...
  handleInputChange(e) {
    ...
    if (e.target.id === 'program_number_flag' && e.target.value === String(this.getNumberCPUs())) {
      ...
      if (e.target.value) {
        this.props.store.buildArgs[e.target.id] = e.target['data-flag'] + ' ' + e.target.value
      } else {
        delete(this.props.store.buildArgs[e.target.id])
      }
    }
