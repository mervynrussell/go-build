...
export default {
  ...
  activate(state) {
    console.log("go-build activating, state=" + state)
    isStarted = false
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      atom.workspace.addOpener(uri => {
        ...
        if (uri === 'atom://go-build') {
          return new GoBuildView({store: this.getStore()});
        }
      }),
  ...
  consumeGoConfig (service) {
    console.log("Consuming GoConfig")
    goconfig = service
  },

  consumeGoGet (service) {
    console.log("Consuming GoGet")
    goget = service
  },
  ...
  getBuilder() {
    console.log("go-build getBuilder")
    if(!connection) {
      ...
      connection = new BuildConnection(
        (args, options) => {
          ...
          return promise.then(() => {
            return spawn("/usr/local/go/bin/go", ['help', 'build'], options)
          })
        },
        (message) => {
          store.dispatch({
  ...
  deserializeGoBuildView(serialized) {
    console.log('deserializeGoBuildView' + JSON.stringify(serialized))
    var store = this.getStore()
    store.buildArgs = serialized.data ? serialized.data : {}
