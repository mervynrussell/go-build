...
export default class Builder {
  ...
  start (config, file) {
    ...
    // start the build
    this._addOutputMessage(`Starting build...\n`)
    ...
    return this._connection.start({ config, file })
      ...
      .catch((err) => {
        this._addOutputMessage(`Failed to start go build\n` + err)
        return this.stop()
      })
  ...
  clean(config, file) {
    ...
    // start cleaning
    this._addOutputMessage(`Cleaning...\n`)
  }
  ...
  stop () {
    ...
    if (!this._stopPromise) {
      // go build is currently running
      const requiresHalt = this.isBusy()
      this._stopPromise = this._connection.stop()
        .then(() => {
          this._stopPromise = null
